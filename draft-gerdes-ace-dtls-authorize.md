...
In PreSharedKey mode, C MAY include a `cnf` object carrying an
identifier for a symmetric key in the Client-to-AS-Request to be used
by AS to construct the proof-of-possession token. This identifier MUST
specify a symmetric key that was previously generated by AS as a
session key for the communication between C and RS.
...
An example Access Token request from C to AS is depicted in
